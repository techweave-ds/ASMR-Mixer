...
export function FavoritesContent() {
  ...
  const favoriteSounds = useMemo(() => sounds.filter((s) => soundIds.includes(s.id)), [soundIds])
  const favoriteCollections = useMemo(() => collections.filter((c) => collectionIds.includes(c.id)), [collectionIds])
  ...
  if (!hasFavorites) return (
    <div className="flex flex-col items-center justify-center h-full text-text-tertiary">
      ...
      <p className="mt-1 text-sm text-text-tertiary text-center max-w-sm">Tap the heart icon on sounds and collections to save them here</p>
    </div>
  )
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-3">
        <div className="h-10 w-10 rounded-lg bg-gradient-to-br from-red-500/20 to-pink-600/20 flex items-center justify-center">
      ...
      {favoriteSounds.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
            {favoriteSounds.map((sound) => (<SoundCard key={sound.id} {...sound} />))}
          </div>
        </section>
      )}
      {favoriteCollections.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {favoriteCollections.map((col) => (<CollectionCard key={col.id} {...col} />))}
          </div>
        </section>
      )}
    </div>
