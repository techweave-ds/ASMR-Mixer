...
export function CollectionCard({
  id, title, description, gradient, duration, coverUrl, tags, soundIds, isPremium, className,
}: CollectionCardProps) {
      ...
      <div className={cn("relative aspect-[3/2] bg-gradient-to-br", gradient)}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          ...
          <div className="flex items-center gap-3 mt-2">
            <span className="flex items-center gap-1 text-[11px] text-text-tertiary"><Music size={10} />{soundIds.length} sounds</span>
            <span className="flex items-center gap-1 text-[11px] text-text-tertiary"><Clock size={10} />{formatDuration(duration)}</span>
          </div>
          <div className="flex flex-wrap gap-1 mt-2">
            {tags.slice(0, 3).map((tag) => (
              <span key={tag} className="rounded-full bg-white/10 px-2 py-0.5 text-[9px] text-text-secondary">{tag}</span>
            ))}
          </div>
        </div>
        ...
        <button onClick={(e) => { e.stopPropagation(); toggleCollection(id) }}
          className={cn("absolute top-3 right-3 flex h-8 w-8 items-center justify-center rounded-full transition-all duration-300",
        ...
        {isPremium && (
          <div className="absolute top-3 left-3 rounded-full bg-gradient-to-r from-amber-500/80 to-orange-500/80 px-2 py-0.5 text-[8px] font-bold text-white uppercase tracking-wider">Premium</div>
        )}
      </div>
    </div>
  )
