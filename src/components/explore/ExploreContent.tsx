  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative max-w-lg">
        ...
        <input type="text" placeholder="Search sounds, categories, tags..." value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          className="w-full rounded-xl border border-border bg-glass py-3 pl-11 pr-4 text-sm text-text-primary outline-none transition-all placeholder:text-text-quaternary focus:border-accent/50" />
      </div>
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setSelectedCategory(null)}
          className={cn("rounded-full px-4 py-1.5 text-xs font-medium transition-all border",
        ...
        {categories.map((cat) => (
          <button key={cat} onClick={() => setSelectedCategory(selectedCategory === cat ? null : cat)}
            className={cn("rounded-full px-4 py-1.5 text-xs font-medium capitalize transition-all border",
