...
export function HomeContent() {
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional client-only randomized personalization
  useEffect(() => { setReturnVisit(Math.random() > 0.5) }, [])
  ...
  const handleHeroWheel = useCallback((e: React.WheelEvent) => {
    ...
    if (el && !heroFadedOut) {
      el.scrollTo({ top: el.scrollTop + e.deltaY })
      e.preventDefault()
    }
